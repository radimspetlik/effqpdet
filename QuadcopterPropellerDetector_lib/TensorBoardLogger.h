/** Copied from https://github.com/RustingSword/tensorboard_logger/ */
   ...
   public:
    ...
    explicit TensorBoardLogger(const std::string &log_file,
        ...
        if (basename.find("tfevents") == std::string::npos) {
            std::cerr << "A valid event file must contain substring \"tfevents\" in its "
                "basename, got " + basename << std::endl;
        ...
        ofs_ = new std::ofstream(
            log_file, std::ios::out |
                          ...
                          std::ios::binary);
        if (!ofs_->is_open()) {
            std::cerr << "failed to open log_file " + log_file << std::endl;
